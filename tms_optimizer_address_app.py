...
# Ứng dụng TMS tối ưu hóa tuyến đường (Tên + Địa chỉ + Tọa độ)
# ===============================================
...
def compute_distance_matrix(coords):
    """Tạo ma trận khoảng cách (mét)."""
    n = len(coords)
    dist = [[0] * n for _ in range(n)]
    for i in range(n):
        for j in range(n):
            if i != j:
                dist[i][j] = int(haversine(coords[i][0], coords[i][1], coords[j][0], coords[j][1]) * 1000)
    return dist


def solve_with_ortools(distance_matrix):
    """Tối ưu tuyến bằng OR-Tools."""
...
def nearest_neighbor(distance_matrix):
    """Thuật toán đơn giản tìm đường đi gần nhất."""
    n = len(distance_matrix)
    visited = [False] * n
    route = [0]
    visited[0] = True
...
with st.sidebar:
    ...
    warehouse_address = st.text_input("Địa chỉ kho", "16A Lê Hồng Phong, Phường 12, Quận 10, TP.HCM")
    warehouse_lat = st.number_input("Vĩ độ (lat) kho", value=10.7755551)
    warehouse_lon = st.number_input("Kinh độ (lon) kho", value=106.6707127)

    st.subheader("2️⃣ Danh sách điểm giao hàng")
    st.markdown("Nhập thủ công hoặc tải file CSV có cột: **Tên, Địa chỉ, Lat, Lon**")

    sample = """Khách A,43 Nguyễn Huệ,10.7755551,106.6707127
Khách B,1 Lê Duẩn,10.7863858,106.7050126
Khách C,60 Lý Tự Trọng,10.7754,106.7032
Khách D,500 Điện Biên Phủ,10.7797,106.6896"""
...
if uploaded:
    ...
    df = pd.read_csv(StringIO(addresses_text), names=["Tên", "Địa chỉ", "Lat", "Lon"])
...
if st.button("🚀 Tối ưu hóa tuyến"):
    ...
    view = pdk.ViewState(
        latitude=np.mean([c[0] for c in all_coords]),
    ...
    layer_points = pdk.Layer(
        ...
        get_radius=80, get_fill_color=[255, 128, 0],
        pickable=True
    )
    ...
    # ---------- TẢI FILE KẾT QUẢ ----------
    st.download_button("⬇️ Tải kết quả CSV", df_result.to_csv(index=False).encode("utf-8"), "route_result.csv", "text/csv")
